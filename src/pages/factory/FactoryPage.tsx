        ...
        <div className="flex items-start justify-between gap-4 mb-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              {typeof project.numero === 'number' && (
                <span
                  className="shrink-0 rounded-md bg-muted px-1.5 py-0.5 font-mono text-xs font-semibold text-muted-foreground"
...
const FactoryPage = () => {
  ...
  // (se terminó la campaña y el borrador se borró, o se avanzó y hay más que retomar).
  const [borrador, setBorrador] = useState(() => leerBorrador());
  useEffect(() => { if (!createOpen) setBorrador(leerBorrador()); }, [createOpen]);
  ...
  const filteredProjects = projects.filter((p) => {
    // La búsqueda también encuentra por estratega (coherente con que sea filtrable) y por número
    // de campaña, con o sin "#": si el número se muestra para identificarla, hay que poder buscarlo.
    const matchesQ = query.trim()
      ? `${p.name} ${p.client} ${p.strategistName ?? ''} #${p.numero ?? ''} ${p.numero ?? ''}`
          .toLowerCase()
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={`${projectsCollapsed ? 'w-11' : 'w-72'} shrink-0 border-r border-border/60 flex flex-col bg-card/40 transition-all duration-300`}>
            ...
            <div className="flex-1 overflow-y-auto py-2 px-2 space-y-3">
              {/* Campaña a medio crear. El asistente ya guardaba el borrador solo, pero no había
                  nada que avisara: había que acordarse de abrir "Nueva campaña" para retomarlo. */}
              {borrador && (
                <div className="rounded-lg border border-dashed border-factory/40 bg-accent/40 px-3 py-2.5">
                  <div className="flex items-center gap-1.5">
                    <FilePen className="h-3.5 w-3.5 text-factory shrink-0" />
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-factory">Campaña sin terminar</p>
                  </div>
                  <p className="mt-1 text-xs font-medium truncate" title={borrador.nombre || 'Sin nombre todavía'}>
                    {borrador.nombre || <span className="italic text-muted-foreground">Sin nombre todavía</span>}
                  </p>
                  {hace(borrador.guardadoEn) && (
                    <p className="text-[10px] text-muted-foreground">Guardada {hace(borrador.guardadoEn)}</p>
                  )}
                  <div className="mt-2 flex gap-1.5">
                    <Button size="sm" className="h-7 text-xs flex-1" onClick={() => setCreateOpen(true)}>
                      Continuar
                    </Button>
                    <Button
                      size="sm"
                      ...
                      onClick={() => { borrarBorrador(); setBorrador(null); }}
                    >
                      Descartar
                    </Button>
                  </div>
                          ...
                          >
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {typeof p.numero === 'number' && (
                                <span className="shrink-0 font-mono text-[10px] font-semibold text-muted-foreground">#{p.numero}</span>
                              )}
